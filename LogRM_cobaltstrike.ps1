function LogRM {
      ...
      #>  
      [CmdletBinding()]
	  Param([string]$domain,[string]$user,[string]$pass,[string]$ip,[switch]$remove,[string]$newest,[int]$eventID,[string]$fips,[switch]$scheduler,[string]$task,[datetime]$at,[int]$loop,[datetime]$stoptime)
      ...
      if($ip -match '127.0.0.1')
      {
        ...
        if($eventID -eq "")
        {
            $events=(4624,4625,4647,4728,4732,4733,4738,4776,4688)
           
            #query all event id's and print the results
            for($counter=0;$counter -lt $events.length;$counter++)
            {
                $eventID=$events[$counter]
                start-Job -ScriptBlock ${function:logQuery} -ArgumentList $eventID,$newest
                    ...
                    try
                    {
                        ...
                        if(Get-PSSession)
                        {
                            ...
                            if($WinrmId -or !$WinrmId)
                            {
                                  ...
                                  if($eventID -eq 4625){$eventID=4625}elseif($eventID -eq 4624){$eventID=4624}elseif($eventID -eq 4647){$eventID=4647}elseif($eventID -eq 4732){$eventID=4732}elseif($eventID -eq 4733){$eventID=4733}elseif($eventID -eq 4738){$eventID=4738}elseif($eventID -eq 4776){$eventID=4776}elseif($eventID -eq 4688){$eventID=4688}elseif($eventID -eq 4728){$eventID=4728}else{$eventID}
...
function logQuery($eventID,$newest,$users)
{
    ...
    if($eventID -eq 4624)
    {
		...
		Get-EventLog -Newest $newest -LogName Security -Instanceid 4624 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property TimeGenerated,MachineName,@{Name="Username";Expression={$_.ReplacementStrings[5]}},@{Name="SourceIP";Expression={$_.ReplacementStrings[18]}},@{Name="SourcePort";Expression={$_.ReplacementStrings[19]}}
    ...
    elseif($eventID -eq 4625)
    {
		...
		Get-EventLog -Newest $newest -LogName Security -Instanceid 4625 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property TimeGenerated,MachineName,@{Name="Username";Expression={$_.ReplacementStrings[5]}},@{Name="SourceIP";Expression={$_.ReplacementStrings[19]}},@{Name="SourcePort";Expression={$_.ReplacementStrings[20]}}
	
	}
    elseif($eventID -eq 4647)
    {
        Write-Host "[+] " -ForegroundColor Green -Nonewline; Write-Output "Information EventID (4647) - User initiated logoff"
        Get-EventLog -Newest $newest -LogName Security -Instanceid 4647 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property TimeGenerated,MachineName,@{Name="Username";Expression={$_.ReplacementStrings[1]}}
    }
    ...
    elseif($eventID -eq 4733)
    {
        ...
        Get-EventLog -Newest $newest -LogName Security -Instanceid 4733 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property TimeGenerated,MachineName,@{Name="AccountName";Expression={$_.ReplacementStrings[6]}},@{Name="Removed User (SPN)";Expression={$_.ReplacementStrings[0]}},@{Name="Removed User SID";Expression={$_.ReplacementStrings[1]}},@{Name="GroupName";Expression={$_.ReplacementStrings[2]}},@{Name="Group SID";Expression={$_.ReplacementStrings[4]}}
    ...
    elseif($using:eventID -eq 4738)
    {
        ...
        Get-EventLog -Newest $newest -LogName Security -Instanceid 4738 | Where {$_.message -notmatch "Account Name:\s*\w+\$"}
    ...
    elseif($eventID -eq 4776)
    {
        ...
        $get4776=Get-EventLog -Newest $newest -LogName Security -Instanceid 4776 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} |  Select -Property TimeGenerated,PSComputerName,MachineName,@{Name="Logon Account";Expression={$_.ReplacementStrings[1]}},@{Name="ComputerName (Source)";Expression={$_.ReplacementStrings[2]}}#,@{Name="Error Type";Expression={$_.ReplacementStrings[3]}}        
        Write-Output "`n"
    ...
    elseif($eventID -eq 4728)
    {
        ...
        $groupHistory=Get-EventLog -Newest $newest -LogName Security -Instanceid 4728 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property TimeGenerated,@{Name="Account Name";Expression={$_.ReplacementStrings[0]}},@{Name="Security ID ";Expression={$_.ReplacementStrings[1]}}, @{Name="Added to Security Group";Expression={$_.ReplacementStrings[2]}}
        ...
        for($c=0;$c -lt $groupUsers.length;$c++)
        {
           ...
           $userBelongToGroups=Invoke-Command -ScriptBlock {'powershell.exe c:\windows\system32\cmd.exe /c net user $accountName /domain' | Select-String "Global Group"}
           $userComment=Invoke-Command -ScriptBlock {'powershell.exe c:\windows\system32\cmd.exe /c net user $accountName /domain' | Select-String -Pattern "^comment"}
    ...
    else <# Executing all event logs if the user doesn't specify one #>
    {
        ...
        Write-Host "[+] " -ForegroundColor Green -Nonewline; Write-Output "Information EventID (4776) - The domain controller attempted to validate the credentials for an account"
        Get-EventLog -Newest $newest -LogName Security -Instanceid 4776 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property TimeGenerated,PSComputerName,MachineName,@{Name="Logon Account";Expression={$_.ReplacementStrings[1]}},@{Name="ComputerName (Source)";Expression={$_.ReplacementStrings[2]}},@{Name="Error Type";Expression={$_.ReplacementStrings[3]}}
    ...
    if($eventID -eq 4624) 
    {
        ...
        Get-EventLog -Newest $newest -LogName Security -Instanceid 4624 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property @{Label="Usernames";Expression={$_.ReplacementStrings[5]}} | Group-Object 'Usernames' | Select-Object @{L='Valid Usernames';E={$_.Name}}
        #Get-EventLog -Newest  $newest -LogName Security -Instanceid 4625 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property @{Label="Username";Expression={$_.ReplacementStrings[5]}},@{Label="Status";Expression={if($_.ReplacementStrings[9] -eq "0xC0000064") { "Username doesn't exist!"} elseif ($_.ReplacementStrings[9] -eq "0xC000006A") {"Username is correct but the Password is wrong!"} elseif ($_.ReplacementStrings[9] -eq "0xC0000072") {"User is currently disabled!"} elseif ($_.ReplacementStrings[9] -eq "0xC0000234") {"User is currently Locked Out!"} }} | Group-Object Username,Status
    ...
    elseif($eventID -eq 4625)                               
    {
        ...
        Get-EventLog -Newest  $newest -LogName Security -Instanceid 4625 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property @{Label="Username";Expression={$_.ReplacementStrings[5]}},@{Label="Status";Expression={if($_.ReplacementStrings[9] -eq "0xC000006A") {"Username is correct but the Password is wrong!"} }} | Where-Object {$_.Status -eq "Username is correct but the Password is wrong!"} | Group-Object Username | Select-Object @{L='Valid Usernames';E={$_.Name}}
    }
    else
    {
        Get-EventLog -Newest $newest -LogName Security -Instanceid 4776 | Where {$_.message -notmatch "Account Name:\s*\w+\$"} | Select -Property @{Label="Username";Expression={$_.ReplacementStrings[1]}},@{Label="Error Type";Expression={$_.ReplacementStrings[3]}},@{Label="Status";Expression={if($_.ReplacementStrings[3] -eq "0xC0000064") { "Username doesn't exist!"} elseif($_.ReplacementStrings[3] -eq "0x0") { "Successful Authentication"} elseif ($_.ReplacementStrings[3] -eq "0xC000006A") {"Username is correct but the Password is wrong!"} elseif ($_.ReplacementStrings[3] -eq "0xC0000072") {"User is currently disabled!"} elseif ($_.ReplacementStrings[3] -eq "0xC0000234") {"User is currently Locked Out!"} }} | Group-Object Username,'Error Type',Status | Select-Object @{L='User information';E={$_.Name}}
    }
...
function runAS($ip,$user,$pass)
{

$pw = ConvertTo-SecureString -string $pass -AsPlainText -Force;
$pp = New-Object -typename System.Management.Automation.PSCredential -argumentlist $user, $pw;
Invoke-Command -Credential $pp -ScriptBlock {} -ComputerName $ip
